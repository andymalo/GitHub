...
def search_command():
    lst.delete(0, END)
    for row in Database.searchEntry(entryTitle.get(),
                                    entryAuthor.get(), entryYear.get(), entryISBN.get()):
        lst.insert(END, row)

def add_command():
    Database.addEntry(entryTitle.get(), entryAuthor.get(), entryYear.get(), entryISBN.get())
    lst.delete(0, END)
...
def delete_command():
    Database.delEntry(selected_tuple[0])

def update_command():
    Database.updateEntry(selected_tuple[0], entryTitle.get(),
                         entryAuthor.get(), entryYear.get(), entryISBN.get())
